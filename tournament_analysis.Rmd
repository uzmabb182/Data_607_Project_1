...
for (i in (1:nrow(temp_data))){
  ...
  opponent_vector <- temp_data[1, ]
  
  opponent_vector <- str_extract(opponent_vector, "\\d+")
  
  opponent_vector <- trimws(opponent_vector, which = c("both"))

  for (j in (1:length(opponent_vector))){
    
    for (k in (1:nrow(new_df))){
      
      if new_df$player_number
    }
  } 
}
